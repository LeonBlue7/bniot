...
判断逻辑：若参数数据包含 `108`、`109` 或 `110` 任一参数，则为 V20。
...
判断逻辑：Ver >= 20 视为 V20，Ver >= 10 视为 V10。

#### 默认版本（最低优先级）
若特征参数和 Ver 字段都无法识别，默认为 V10。
...
设备上报的原始数据单位与平台显示单位不同，需进行转换：

### 实时数据字段

| 字段 | 设备上报单位 | 平台显示单位 | 转换规则 |
|------|-------------|-------------|---------|
| `temp` | °C | °C | 无需转换 |
| `humi` | % | % | 无需转换 |
| `current` | mA | A | 除以 1000 |
| `csq` | dB | dB | 无需转换 |
| `airstate` | 状态码 | 状态码 | 无需转换 |

### 转换实现位置

- **存储转换**：`backend/app/mqtt/handlers.py` - MQTT 数据入库时转换
- **查询转换**：`backend/app/api/endpoints/devices.py` - API 返回时转换（兼容旧数据）

### 重要注意事项

1. **电流值转换**：设备上报电流单位为 mA（毫安），如 215mA，需转换为 0.215A 显示
2. **兼容旧数据**：API 查询时也需要转换，确保数据库中旧数据正确显示
3. **新增字段**：如设备上报新数据字段，必须确认其单位并在文档中记录
